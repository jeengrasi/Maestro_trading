...
async def call_openrouter(model: str, system_prompt: str, message: str, timeout: float = 45.0) -> str:
    ...
    payload = {"model": "openrouter/free", "messages": [{"role": "system", "content": system_prompt}, {"role": "user", "content": message}]}
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    data = response.json()
                    logger.info(f"OpenRouter | Latencia: {time.perf_counter() - start_time:.2f}s")
