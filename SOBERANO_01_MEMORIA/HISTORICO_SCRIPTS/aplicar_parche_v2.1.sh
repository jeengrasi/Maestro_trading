...
echo "🔧 APLICANDO PARCHE DE SINTAXIS Y BLINDAJE PYCACHE..."

# 1. Agregar __pycache__/ al .gitignore
if ! grep -q "__pycache__" .gitignore 2>/dev/null; then
    echo "__pycache__/" >> .gitignore
    echo "*.pyc" >> .gitignore
    echo "✅ .gitignore actualizado con reglas de Python."
fi

# Expulsar pycache si fue rastreado
git rm -r --cached SOBERANO_02_CORE/core/__pycache__ 2>/dev/null || true

# 2. Corregir función calcular_metricas y escape de regex en nexus_cli.sh
sed -i 's/rm -rf \\\~/rm -rf ~/g' SOBERANO_00_GOBIERNO/nexus_cli.sh

# Reemplazar la función de métricas para garantizar enteros limpios
...
mv aplicar_parche_v2.1.sh SOBERANO_01_MEMORIA/HISTORICO_SCRIPTS/ 2>/dev/null || true
git add .gitignore SOBERANO_00_GOBIERNO/ SOBERANO_01_MEMORIA/
git commit -m "[GOBIERNO] Parche CLI v2.1: Arreglo de sintaxis de calculo, regex limpia y pycache ignorado" || echo "Sin cambios."

echo "======================================================"
echo "✅ PARCHE APLICADO Y CLI REFINADO A v2.1"
echo "======================================================"
